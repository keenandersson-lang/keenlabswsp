...
const categoryMeta: Record<
  FailureCategory,
  { label: string; retryable: boolean; maxRetries: number; backoffMs: number }
> = {
...
async function handleDateBackfill(body: Record<string, any>) {
  ...
  if (batch.length === 0) {
    return jsonRes({
      ok: true,
      message: "All trading days backfilled.",
      completedDays: tradingDays.length,
    ...
    try {
      ...
      if (httpStatus === 429) {
        ...
        try {
          ...
          lastError = `JSON parse failed on retry for ${date}`;
          break;
        }
        ...
        if (resultCount === 0) {
          perDateLog.push({
            ...
            rows: 0,
            httpStatus,
            error: `status=${data.status}, queryCount=${data.queryCount}`,
          });
          completedDays++;
          lastDate = date;
        } else {
...
async function handleSymbolBackfill(body: Record<string, any>) {
  ...
  if (specificSymbols?.length) {
    ...
    const { data: existingCoverage } = await supabase.from("daily_prices").select("symbol").in("symbol", TIER1_SYMBOLS);
    ...
    symbolsToProcess = TIER1_SYMBOLS.filter((s) => (barCounts[s] ?? 0) < expectedDays * 0.8).sort((a, b) => {
    ...
    const { data: existingCoverage } = await supabase.from("daily_prices").select("symbol");
  ...
  return jsonRes({
    ...
    topFailureCategories: Object.entries(failureCounts)
      ...
      .map(([cat, count]) => ({
        category: cat,
        count,
        retryable: categoryMeta[cat as FailureCategory]?.retryable ?? false,
      })),
...
async function upsertWithRetry(symbol: string, batch: any[]): Promise<{ ok: boolean; written: number }> {
  ...
  for (let attempt = 0; attempt <= meta.maxRetries; attempt++) {
    const { error, count } = await supabase.from("daily_prices").upsert(
      batch.map((b: any) => ({
        symbol,
        date: b.date,
        open: b.open,
...
function classifyError(err: unknown): { category: FailureCategory; reason: string } {
  ...
  if (lower.includes("timeout") || lower.includes("abort"))
    return { category: "provider_timeout", reason: safeReason(message) };
  if (lower.includes("500") || lower.includes("502") || lower.includes("503"))
    return { category: "provider_5xx", reason: safeReason(message) };
  if (lower.includes("404") || lower.includes("not found"))
    return { category: "symbol_not_found", reason: safeReason(message) };
  return fallback;
}
