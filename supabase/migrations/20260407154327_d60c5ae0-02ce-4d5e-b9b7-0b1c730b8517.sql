...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      WHEN p.wsp_pattern = 'climbing' AND COALESCE(p.wsp_score, 0) >= 4
    ...
    END,
    -- review_needed
    CASE WHEN p.wsp_pattern IN ('climbing', 'base_or_climbing') AND COALESCE(p.wsp_score, 0) BETWEEN 3 AND 3 THEN true ELSE false END,
    -- blocked_low_quality
  ...
  SET
    ...
    status = CASE
      ...
      ELSE 'completed'
    END,
    metadata = jsonb_build_object('universe_run_id', v_universe_run_id, 'rule_version', 'phase10_unified_v1')
...
AS $function$
  ...
  WHERE recommendation IN ('KÖP', 'BEVAKA')
    AND pattern IN ('climbing', 'base_or_climbing')
  ORDER BY
    CASE WHEN recommendation = 'KÖP' THEN 0 ELSE 1 END,
    score DESC,
    (payload->>'volume_ratio')::numeric DESC NULLS LAST
  LIMIT 10;
$function$;
...
AS $function$
  SELECT jsonb_build_object(
    ...
    'base', COUNT(*) FILTER (WHERE pattern IN ('base_or_climbing', 'base')),
    'tired', COUNT(*) FILTER (WHERE pattern = 'tired'),
