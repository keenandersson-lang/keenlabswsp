-- Fix get_equity_snapshots to only count the 11 canonical GICS sectors
  ...
  industry_counts AS (
    SELECT
      ...
      COUNT(DISTINCT r.industry) FILTER (
        ...
        AND r.sector IN (SELECT sector_name FROM gics_sectors)
  ...
  sector_counts AS (
    SELECT
      ...
      COUNT(DISTINCT r.sector) FILTER (
        WHERE r.sector IN (SELECT sector_name FROM gics_sectors)
  ...
  reference_counts AS (
    SELECT
      COUNT(DISTINCT canonical_sector) FILTER (
        WHERE canonical_sector IN (SELECT sector_name FROM gics_sectors)
      ...
      COUNT(DISTINCT canonical_industry) FILTER (
        ...
        AND canonical_sector IN (SELECT sector_name FROM gics_sectors)
...
-- Fix coverage report to only count GICS sectors
...
AS $function$
  ...
  SELECT jsonb_build_object(
    ...
    'coverage', jsonb_build_object(
      ...
      'distinct_sectors_in_universe', (SELECT COUNT(DISTINCT canonical_sector) FROM symbols WHERE is_active = true AND canonical_sector IN (SELECT sector_name FROM gics_sectors)),
      'distinct_industries_in_universe', (SELECT COUNT(DISTINCT canonical_industry) FROM symbols WHERE is_active = true AND canonical_industry IS NOT NULL AND canonical_industry != 'Unknown' AND canonical_industry != '' AND canonical_sector IN (SELECT sector_name FROM gics_sectors)),
      'distinct_industries_in_latest_scan', (SELECT COUNT(DISTINCT industry) FROM market_scan_results_latest WHERE industry IS NOT NULL AND industry != 'Unknown' AND industry != '' AND sector IN (SELECT sector_name FROM gics_sectors)),
      'equities_with_industry_metadata', (SELECT COUNT(*) FROM symbols WHERE is_active = true AND canonical_industry IS NOT NULL AND canonical_industry != 'Unknown' AND canonical_industry != '' AND canonical_sector IN (SELECT sector_name FROM gics_sectors))
    ),
