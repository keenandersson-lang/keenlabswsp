...
DECLARE
  ...
  v_got_lock boolean;
BEGIN
  -- OVERLAP GUARD: try to acquire advisory lock (key 8675309 = backfill)
  -- pg_try_advisory_lock returns immediately, no blocking
  SELECT pg_try_advisory_lock(8675309) INTO v_got_lock;
  ...
  IF NOT v_got_lock THEN
    INSERT INTO public.data_sync_log (
      ...
      jsonb_build_object(
        ...
        'skip_reason', 'already_running_advisory_lock'
      ),
...
EXCEPTION WHEN OTHERS THEN
  ...
  IF v_log_id IS NOT NULL THEN
    ...
    WHERE id = v_log_id;
  END IF;
  -- Release advisory lock on error too
  PERFORM pg_advisory_unlock(8675309);
END;
$function$;
