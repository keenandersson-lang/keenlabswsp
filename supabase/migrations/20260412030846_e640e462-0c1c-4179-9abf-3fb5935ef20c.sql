CREATE OR REPLACE FUNCTION public.refresh_scanner_universe_snapshot(p_as_of_date text, p_run_label text)
RETURNS bigint
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  symbol_base AS (
    SELECT
      ...
      -- indicator_ready requires 50+ bars AND valid ma50/volume_ratio per WSP data contract
      (COALESCE(pc.history_bars, 0) >= 50
        AND li.calc_date IS NOT NULL AND li.ma50 IS NOT NULL AND li.volume_ratio IS NOT NULL) AS indicator_ready,
  ...
  SET
    ...
    metadata = jsonb_build_object('as_of_date', p_as_of_date, 'rule_version', 'phase7_v5_50bar_gate', 'baseline_eligible', counts.baseline_count)
  FROM (
