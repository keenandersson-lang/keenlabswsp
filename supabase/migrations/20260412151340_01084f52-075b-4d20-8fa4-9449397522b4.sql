...
-- Promote all active common stocks on major exchanges with valid GICS sectors to core + eligible
...
WHERE is_active = true
  AND is_common_stock = true
  AND is_etf = false
  AND (is_adr IS NULL OR is_adr = false)
  AND primary_exchange IN ('NYSE','NASDAQ','AMEX','ARCA')
