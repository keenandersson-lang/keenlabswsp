...
-- 1. Add resistance_level column
ALTER TABLE public.wsp_indicators ADD COLUMN IF NOT EXISTS resistance_level numeric;

-- 2. Drop existing function (return type changed previously, need clean slate)
DROP FUNCTION IF EXISTS public.materialize_wsp_indicators_from_prices(date, integer, text[]);

-- 3. Recreate materialization function with resistance_level
...
DECLARE
  ...
  v_ref_symbols text[] := ARRAY['SPY','XLK','XLF','XLV','XLE','XLY','XLI','XLC','XLP','XLB','XLRE','XLU'];
BEGIN
  ...
  ref_symbols AS (
    SELECT unnest(v_ref_symbols) AS symbol
  ),
  all_price_symbols AS (
    SELECT symbol FROM target_symbols
    UNION
    SELECT symbol FROM ref_symbols
  ),
  source_prices AS (
    SELECT
      dp.symbol, dp.date,
      ...
      MAX(dp.high::numeric) OVER (PARTITION BY dp.symbol ORDER BY dp.date ROWS BETWEEN 251 PRECEDING AND CURRENT ROW) AS resistance_level,
    ...
    JOIN all_price_symbols aps ON aps.symbol = dp.symbol
  ...
  final_rows AS (
    ...
    WHERE ws.rn = 1
      ...
      AND ws.symbol IN (SELECT symbol FROM target_symbols)
  ),
  with_pattern AS (
    SELECT fr.*,
      CASE
  ...
  upserted AS (
    INSERT INTO public.wsp_indicators (
      ...
      pct_change_1d, pct_from_52w_high, mansfield_rs, mansfield_rs_sector,
    ...
    SELECT
      ...
      wp.pct_change_1d, wp.pct_from_52w_high, wp.mansfield_rs, wp.mansfield_rs_sector,
      wp.resistance_level, now()
...
-- 4. Update run_broad_market_scan with breakout detection
DROP FUNCTION IF EXISTS public.run_broad_market_scan(date, text);

CREATE FUNCTION public.run_broad_market_scan(
  p_as_of_date date,
  p_run_label text
)
RETURNS bigint
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- BREAKOUT DETECTION: close > resistance_level * 1.02 with volume >= 2x
      v_breakout_detected := false;
      IF v_resistance_level IS NOT NULL AND v_resistance_level > 0
         AND v_close > v_resistance_level * 1.02
         AND COALESCE(v_volume_ratio, 0) >= 2.0
      THEN
        v_breakout_detected := true;
      END IF;

      -- RECOMMENDATION: KÖP requires climbing + score 5 + breakout
      IF v_above_ma150 IS NOT NULL AND NOT v_above_ma150 THEN
        v_recommendation := 'SÄLJ';
      ELSIF v_pattern = 'climbing' AND v_score = 5 AND v_breakout_detected THEN
