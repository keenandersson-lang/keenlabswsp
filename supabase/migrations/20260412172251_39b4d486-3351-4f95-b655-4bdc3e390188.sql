...
-- Drop old signatures to avoid PostgREST ambiguity
DROP FUNCTION IF EXISTS public.get_equity_screener_rows(integer, integer, text);
DROP FUNCTION IF EXISTS public.get_top_wsp_setups();

-- Recreate get_equity_screener_rows with filter params
...
-- Update get_top_wsp_setups to deprioritize Unknown industry
...
AS $function$
  ...
  ORDER BY
    CASE r.recommendation
      WHEN 'KÖP' THEN 0 WHEN 'BEVAKA' THEN 1 WHEN 'AVVAKTA' THEN 2 WHEN 'SÄLJ' THEN 3 ELSE 4 END,
    -- Deprioritize Unknown/missing industry
